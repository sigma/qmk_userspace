...
- **Music layers (experimental).** Four app layers reached from the `_SYSTEM`
  picker: `_MIDI` (10) puts a whole one-octave, piano-style keyboard on the
  **right hand** — home row = white keys (C–B), QWERTY row = each sharp directly
  above its natural (C#/D# `_` F#/G#/A#) — with relative notes. The **left hand**
  holds the controls: velocity + octave down/up on its home row, transpose above,
  channel / all-notes-off on the number row, mod on the bottom row, and sustain /
  pitch bend on the thumb (all lit in a distinct MIDI colour); `_ABLETON` (9),
  `_RENOISE` (8) and `_TRAKTOR`
